...
docker build -f backend/Dockerfile.prod -t backend-staging backend \
    && docker tag backend-staging ${AWS_ECR_URL}/backend-staging:latest \
    && docker push 630869177434.dkr.ecr.eu-central-1.amazonaws.com/backend-staging:latest &
 
docker build -f frontend/Dockerfile.prod -t frontend-staging \
        --build-arg API_URL=https://dev-api.skoleapp.com/ \
        --build-arg BACKEND_URL=https://dev-api.skoleapp.com/ \
        --build-arg CLOUDMERSIVE_API_KEY=f0d72604-8df2-45e8-9d0b-09029ee16525 frontend \
    && docker tag frontend-staging ${AWS_ECR_URL}/frontend-staging:latest \
    && docker push ${AWS_ECR_URL}/frontend-staging:latest &
